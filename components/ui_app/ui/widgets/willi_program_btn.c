...
#define WILLI_ICON_ORANGE       0xFFAA1D

#define WILLI_PROGRAM_ICON      LV_SYMBOL_SETTINGS

static void willi_program_btn_init_style_main(lv_style_t *s)
{
    lv_style_init(s);
...
static void willi_program_btn_init_style_pressed(lv_style_t *s)
{
    lv_style_init(s);
...
static void willi_program_btn_init_style_checked(lv_style_t *s)
{
    lv_style_init(s);
...
lv_obj_t *willi_program_btn_create(lv_obj_t *parent)
{
    ...
    if(!inited) {
        willi_program_btn_init_style_main(&style_main);
        willi_program_btn_init_style_pressed(&style_pressed);
        willi_program_btn_init_style_checked(&style_checked);
        willi_program_btn_init_style_topline(&style_topline);
        inited = true;
    }
    ...
    lv_obj_set_size(btn, WILLI_PROGRAM_BTN_W, WILLI_PROGRAM_BTN_H);
    ...
    lv_obj_set_size(topline, WILLI_PROGRAM_BTN_W - 20, 2);
    ...
    /* --- contenedor central real del contenido --- */
    ...
    lv_label_set_text(icon, WILLI_PROGRAM_ICON);
    lv_obj_set_style_text_color(icon, lv_color_hex(WILLI_ICON_ORANGE), 0);
